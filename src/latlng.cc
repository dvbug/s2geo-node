...
void LatLng::Init(Local<Object> exports) {
    ...
    // Prepare constructor template
      ...
      // can't read prop val from `PrototypeTemplate`
      // Local<ObjectTemplate> proto = tpl->PrototypeTemplate();
      // proto->SetAccessor(String::NewFromUtf8(isolate, "lat"), Lat);
      // proto->SetAccessor(String::NewFromUtf8(isolate, "lng"), Lng);

      Local<ObjectTemplate> point_inst = tpl->InstanceTemplate();
      point_inst->SetAccessor(String::NewFromUtf8(isolate, "lat"), Lat);
      point_inst->SetAccessor(String::NewFromUtf8(isolate, "lng"), Lng);
      point_inst->SetAccessor(String::NewFromUtf8(isolate, "latRadians"), LatRadians);
      point_inst->SetAccessor(String::NewFromUtf8(isolate, "lngRadians"), LngRadians);
...
void LatLng::LatRadians(v8::Local<v8::String> property, const PropertyCallbackInfo<v8::Value>& args){
    Isolate* isolate = args.GetIsolate();
    LatLng* obj = ObjectWrap::Unwrap<LatLng>(args.Holder());
    args.GetReturnValue().Set(Number::New(isolate,obj->this_.lat().radians()));

}

void LatLng::LngRadians(v8::Local<v8::String> property, const PropertyCallbackInfo<v8::Value>& args){
    Isolate* isolate = args.GetIsolate();
    LatLng* obj = ObjectWrap::Unwrap<LatLng>(args.Holder());
    args.GetReturnValue().Set(Number::New(isolate,obj->this_.lng().radians()));
