...
void CellId::Init(Local<Object> exports) {
  ...
  // Prototype
    ...
    NODE_SET_PROTOTYPE_METHOD(tpl, "getEdgeNeighbors", GetEdgeNeighbors);
    NODE_SET_PROTOTYPE_METHOD(tpl, "getVertexNeighbors", GetVertexNeighbors);
    NODE_SET_PROTOTYPE_METHOD(tpl, "getAllNeighbors", GetAllNeighbors);
...
void CellId::GetEdgeNeighbors(const FunctionCallbackInfo<Value>& args) {
  CellId* obj = node::ObjectWrap::Unwrap<CellId>(args.Holder());
...
void CellId::GetVertexNeighbors(const FunctionCallbackInfo<Value>& args) {
  CellId* obj = node::ObjectWrap::Unwrap<CellId>(args.Holder());
  vector<S2CellId> neighbors;
  obj->this_.AppendVertexNeighbors(args[0]->ToNumber()->Value(), &neighbors);
  Local<Array> neighborsArray = Array::New(Isolate::GetCurrent(), neighbors.size());
  args.GetReturnValue().Set(neighborsArray);
...
void CellId::GetAllNeighbors(const FunctionCallbackInfo<Value>& args) {
  CellId* obj = node::ObjectWrap::Unwrap<CellId>(args.Holder());
  vector<S2CellId> neighbors;
  obj->this_.AppendAllNeighbors(args[0]->ToNumber()->Value(), &neighbors);
  Local<Array> neighborsArray = Array::New(Isolate::GetCurrent(), neighbors.size());
  args.GetReturnValue().Set(neighborsArray);
  for (int ii = 0; ii < neighbors.size(); ++ii) {
    neighborsArray->Set(ii, CellId::CreateNew(args, neighbors[ii]));
